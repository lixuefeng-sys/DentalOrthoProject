...
vtkStandardNewMacro(ConstrainedPlaneWidget);

vtkCxxSetObjectMacro(ConstrainedPlaneWidget, PlaneProperty, vtkProperty);

ConstrainedPlaneWidget::ConstrainedPlaneWidget()
  : vtkPolyDataSourceWidget()
{
  this->State = ConstrainedPlaneWidget::Start;
  this->EventCallbackCommand->SetCallback(ConstrainedPlaneWidget::ProcessEvents);
...
void ConstrainedPlaneWidget::SetEnabled(int enabling)
{
  if (!this->Interactor)
  {
...
//------------------------------------------------------------------------------
void ConstrainedPlaneWidget::RegisterPickers()
{
...
void ConstrainedPlaneWidget::ProcessEvents(
  vtkObject* vtkNotUsed(object), unsigned long event, void* clientdata, void* vtkNotUsed(calldata))
{
  ConstrainedPlaneWidget* self = reinterpret_cast<ConstrainedPlaneWidget*>(clientdata);
...
void ConstrainedPlaneWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
void ConstrainedPlaneWidget::PositionHandles()
{
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  double* pt2 = this->PlaneSource->GetPoint2();
...
int ConstrainedPlaneWidget::HighlightHandle(vtkProp* prop)
{
...
void ConstrainedPlaneWidget::HighlightNormal(int highlight)
{
  if (highlight)
  {
    this->ValidPick = 1;
...
void ConstrainedPlaneWidget::HighlightPlane(int highlight)
{
  if (highlight)
  {
    this->ValidPick = 1;
...
void ConstrainedPlaneWidget::OnStartPinch()
{
  ...
  {
    this->State = ConstrainedPlaneWidget::Outside;
    return;
  }
  ...
  if (path != nullptr)
  {
    this->State = ConstrainedPlaneWidget::Pinching;
...
void ConstrainedPlaneWidget::OnPinch()
{
  if (this->State != ConstrainedPlaneWidget::Pinching)
  {
    return;
  }
...
void ConstrainedPlaneWidget::OnEndPinch()
{
  if (this->State != ConstrainedPlaneWidget::Pinching)
  {
    return;
...
void ConstrainedPlaneWidget::OnLeftButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = ConstrainedPlaneWidget::Moving;
  ...
  else
  {
    ...
    if (path != nullptr)
    {
      ...
      {
        this->State = ConstrainedPlaneWidget::Rotating;
        this->HighlightNormal(1);
      }
      else if (this->Interactor->GetControlKey())
      {
        this->State = ConstrainedPlaneWidget::Spinning;
        this->HighlightNormal(1);
      }
      else
      {
...
void ConstrainedPlaneWidget::OnLeftButtonUp()
{
  if (this->State == ConstrainedPlaneWidget::Outside || this->State == ConstrainedPlaneWidget::Start)
  {
    return;
  }
...
void ConstrainedPlaneWidget::OnMiddleButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = ConstrainedPlaneWidget::Pushing;
    this->HighlightPlane(1);
    this->HighlightNormal(1);
    this->HighlightHandle(path->GetFirstNode()->GetViewProp());
  }
...
void ConstrainedPlaneWidget::OnMiddleButtonUp()
{
  if (this->State == ConstrainedPlaneWidget::Outside || this->State == ConstrainedPlaneWidget::Start)
  {
    return;
...
void ConstrainedPlaneWidget::OnRightButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if (path != nullptr)
  {
    this->State = ConstrainedPlaneWidget::Scaling;
    this->HighlightPlane(1);
    this->HighlightHandle(path->GetFirstNode()->GetViewProp());
  }
...
void ConstrainedPlaneWidget::OnRightButtonUp()
{
  if (this->State == ConstrainedPlaneWidget::Outside || this->State == ConstrainedPlaneWidget::Start)
  {
    return;
...
void ConstrainedPlaneWidget::OnMouseMove()
{
  ...
  // Process the motion
  if (this->State == ConstrainedPlaneWidget::Moving)
  {
    // Okay to process
    if (this->CurrentHandle)
    {
    ...
    {
      this->ConstrainedTranslate(prevPickPoint, pickPoint);
    }
  }
  else if (this->State == ConstrainedPlaneWidget::Scaling)
  {
    this->Scale(prevPickPoint, pickPoint, X, Y);
  }
  else if (this->State == ConstrainedPlaneWidget::Pushing)
  {
    this->Push(prevPickPoint, pickPoint);
  }
  else if (this->State == ConstrainedPlaneWidget::Rotating)
  {
    camera->GetViewPlaneNormal(vpn);
    this->Rotate(X, Y, prevPickPoint, pickPoint, vpn);
  }
  else if (this->State == ConstrainedPlaneWidget::Spinning)
  {
...
void ConstrainedPlaneWidget::MoveOrigin(double* p1, double* p2)
{
...
void ConstrainedPlaneWidget::MovePoint1(double* p1, double* p2)
{
  // Get the plane definition
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
...
void ConstrainedPlaneWidget::MovePoint2(double* p1, double* p2)
{
  // Get the plane definition
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
...
void ConstrainedPlaneWidget::MovePoint3(double* p1, double* p2)
{
  // Get the plane definition
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
...
void ConstrainedPlaneWidget::Rotate(int X, int Y, double* p1, double* p2, double* vpn)
{
  double* o = this->PlaneSource->GetOrigin();
  double* pt1 = this->PlaneSource->GetPoint1();
  double* pt2 = this->PlaneSource->GetPoint2();
...
void ConstrainedPlaneWidget::Spin(double* p1, double* p2)
{
  // Mouse motion vector in world space
  double v[3];
  v[0] = p2[0] - p1[0];
...
void ConstrainedPlaneWidget::Translate(double* p1, double* p2)
{
...
// Loop through all points and translate them
void ConstrainedPlaneWidget::ConstrainedTranslate(double* p1, double* p2)
{
  // Get the motion vector
  double v[] = {0, 0, 0};
  if(m_movement == X_Axis || m_movement == FreeMovement)
  {
    v[0] = p2[0] - p1[0];
  }
  if(m_movement == Y_Axis || m_movement == FreeMovement)
  {
    v[1] = p2[1] - p1[1];
  }
  if(m_movement == Z_Axis || m_movement == FreeMovement)
  {
    v[2] = p2[2] - p1[2];
  }
...
void ConstrainedPlaneWidget::Scale(double* p1, double* p2, int vtkNotUsed(X), int Y)
{
  // Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
void ConstrainedPlaneWidget::Push(double* p1, double* p2)
{
  // Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
void ConstrainedPlaneWidget::CreateDefaultProperties()
{
  // Handle properties
...
void ConstrainedPlaneWidget::PlaceWidget(double bds[6])
{
  int i;
...
void ConstrainedPlaneWidget::SizeHandles()
{
...
void ConstrainedPlaneWidget::SelectRepresentation()
{
  if (!this->CurrentRenderer)
  {
    return;
...
void ConstrainedPlaneWidget::SetResolution(int r)
{
  this->PlaneSource->SetXResolution(r);
  this->PlaneSource->SetYResolution(r);
}

int ConstrainedPlaneWidget::GetResolution()
{
...
void ConstrainedPlaneWidget::SetOrigin(double x, double y, double z)
{
  this->PlaneSource->SetOrigin(x, y, z);
  this->PositionHandles();
}

void ConstrainedPlaneWidget::SetOrigin(double x[3])
{
  this->SetOrigin(x[0], x[1], x[2]);
}

double* ConstrainedPlaneWidget::GetOrigin()
{
  return this->PlaneSource->GetOrigin();
}

void ConstrainedPlaneWidget::GetOrigin(double xyz[3])
{
...
void ConstrainedPlaneWidget::SetPoint1(double x, double y, double z)
{
  this->PlaneSource->SetPoint1(x, y, z);
  this->PositionHandles();
}

void ConstrainedPlaneWidget::SetPoint1(double x[3])
{
  this->SetPoint1(x[0], x[1], x[2]);
}

double* ConstrainedPlaneWidget::GetPoint1()
{
  return this->PlaneSource->GetPoint1();
}

void ConstrainedPlaneWidget::GetPoint1(double xyz[3])
{
...
void ConstrainedPlaneWidget::SetPoint2(double x, double y, double z)
{
  this->PlaneSource->SetPoint2(x, y, z);
  this->PositionHandles();
}

void ConstrainedPlaneWidget::SetPoint2(double x[3])
{
  this->SetPoint2(x[0], x[1], x[2]);
}

double* ConstrainedPlaneWidget::GetPoint2()
{
  return this->PlaneSource->GetPoint2();
}

void ConstrainedPlaneWidget::GetPoint2(double xyz[3])
{
...
void ConstrainedPlaneWidget::SetCenter(double x, double y, double z)
{
...
// Description:
// Set the center of the plane.
void ConstrainedPlaneWidget::SetCenter(double c[3])
{
...
double* ConstrainedPlaneWidget::GetCenter()
{
  return this->PlaneSource->GetCenter();
}

void ConstrainedPlaneWidget::GetCenter(double xyz[3])
{
...
void ConstrainedPlaneWidget::SetNormal(double x, double y, double z)
{
...
// Description:
// Set the normal to the plane.
void ConstrainedPlaneWidget::SetNormal(double n[3])
{
...
double* ConstrainedPlaneWidget::GetNormal()
{
  return this->PlaneSource->GetNormal();
}

void ConstrainedPlaneWidget::GetNormal(double xyz[3])
{
  this->PlaneSource->GetNormal(xyz);
}

void ConstrainedPlaneWidget::GetPolyData(vtkPolyData* pd)
{
  pd->ShallowCopy(this->PlaneSource->GetOutput());
}

vtkPolyDataAlgorithm* ConstrainedPlaneWidget::GetPolyDataAlgorithm()
{
  return this->PlaneSource;
}

void ConstrainedPlaneWidget::GetPlane(vtkPlane* plane)
{
  if (plane == nullptr)
  {
    return;
...
void ConstrainedPlaneWidget::UpdatePlacement()
{
  this->PlaneSource->Update();
  this->PositionHandles();
}

void ConstrainedPlaneWidget::SetHandlesOff()
{
  for (int i = 0; i < 4; i++)
  {
...
void ConstrainedPlaneWidget::SetHandlesOn()
{
  for (int i = 0; i < 4; i++)
  {
...
void ConstrainedPlaneWidget::SetNormalOff()
{
  this->LineActor->VisibilityOff();
  this->ConeActor->VisibilityOff();
  this->LineActor2->VisibilityOff();
...
void ConstrainedPlaneWidget::SetNormalOn()
{
...
void ConstrainedPlaneWidget::LimitMovement(enum Movement movement)
{
  m_movement = movement;
}
